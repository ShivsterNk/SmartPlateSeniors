...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['remove_meal_id'])) {
    ...
    $stmtRemove = $pdo->prepare("DELETE FROM favorites WHERE user_id = ? AND meal_id = ?");
...
?>
    <style>
        body { background-color: #FEFAE0; padding-top: 0 !important; }
        .toast {
            position: fixed;
            bottom: 20px;
            right: 20px;
        ...
        .page-header {
            background: linear-gradient(135deg, var(--green-dark), #3a5220);
            color: white;
            padding: 20px 0;
        ...
        .btn-view:hover { background-color: var(--green-dark); color: white; }
        ...
        .btn-explore:hover { background-color: var(--green-dark); color: white; }
